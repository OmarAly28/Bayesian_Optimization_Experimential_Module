...
experiment_history = []
suggested_x = None
TOLERANCE = 1e-4  # Stopping threshold for convergence
CONVERGENCE_PATIENCE = 10  # Number of iterations with little improvement before declaring convergence
patience_counter = 0
best_y_so_far_for_convergence = None
...
def set_ui_state(phase='setup', lock_all=False):
    ...
    warm_restart_button.disabled = is_setup # Disable warm restart in setup phase
...
def lock_in_setup():
    ...
    def worker(config):
        global optimizer, dimensions, param_names, maximize_objective, experiment_history, best_y_so_far_for_convergence, patience_counter
        try:
            ...
            # Initialize convergence tracking for a new run
            patience_counter = 0
            if experiment_history:
                best_y_so_far_for_convergence = max(y for _, y in experiment_history) if maximize_objective else min(y for _, y in experiment_history)
            else:
                best_y_so_far_for_convergence = None
...
def suggest_next_experiment():
    ...
    try:
        ...
        suggest_button.disabled = True # Temporarily disable to prevent multiple clicks
        submit_result_button.disabled = True # Disable submit until suggestion is given
        ...
        suggest_button.disabled = False # Re-enable button on error

def submit_result():
    """Submits the experimental result to the optimizer and updates the display."""
    global suggested_x, patience_counter, best_y_so_far_for_convergence
    ...
    submit_result_button.disabled = True # Disable to prevent double submission
    suggest_button.disabled = True # Keep suggest disabled until model is updated
    ...
    process_and_plot_latest_results()
    check_for_convergence(internal_result) # Check for convergence after processing results
    update_initial_points_warning(len(experiment_history))
    ...
    # Only re-enable suggest button if not converged
    if not suggest_button.disabled: 
        suggest_button.disabled = False

def check_for_convergence(current_internal_y):
    """
    Checks if the optimizer has converged based on the improvement over recent iterations.
    """
    global patience_counter, best_y_so_far_for_convergence

    if best_y_so_far_for_convergence is None:
        best_y_so_far_for_convergence = current_internal_y
        patience_counter = 0
        return

    if maximize_objective:
        # For maximization, we want to see improvement (current_internal_y > best_y_so_far_for_convergence)
        if current_internal_y > best_y_so_far_for_convergence + TOLERANCE:
            best_y_so_far_for_convergence = current_internal_y
            patience_counter = 0
        else:
            patience_counter += 1
    else:
        # For minimization, we want to see improvement (current_internal_y < best_y_so_far_for_convergence)
        if current_internal_y < best_y_so_far_for_convergence - TOLERANCE:
            best_y_so_far_for_convergence = current_internal_y
            patience_counter = 0
        else:
    ...
    if patience_counter >= CONVERGENCE_PATIENCE:
        update_status(f"🎉 Optimizer converged! Best objective value found. Consider a warm restart or reset.", is_error=False)
        suggest_button.disabled = True
        submit_result_button.disabled = True
        warm_restart_button.disabled = False # Enable warm restart after convergence

def perform_warm_restart():
    """
    Performs a warm restart of the optimizer, keeping existing data.
    """
    global optimizer, patience_counter, best_y_so_far_for_convergence

    if optimizer is None:
        update_status("❌ Error: Optimizer not initialized for warm restart.", is_error=True)
        return

    update_status("🔄 Performing warm restart...")
    set_ui_state(lock_all=True) # Temporarily lock UI during restart

    try:
        # Recreate optimizer with the same setup but using the accumulated data
        config = {
            ...
            "surrogate_model": surrogate_select.value,
            "acq_func": acq_func_select.value,
            "params": [{"name": n, "low": d.low, "high": d.high} for n, d in zip(param_names, dimensions)],
            "initial_data": [] # This will be populated from experiment_history
        }

        # Populate initial_data from experiment_history for the new optimizer
        for x_val, y_internal in experiment_history:
            config["initial_data"].append({"x": x_val, "y": -y_internal if maximize_objective else y_internal})

        # The n_initial_points for the new optimizer should be at least the number of current points
        # to prevent it from suggesting random points when it already has data.
        # We also maintain the original logic of max(5, 2 * num_params)
        config["initial_random_points"] = max(
            5,
            2 * config["num_params"],
            len(experiment_history) # Ensure it doesn't try to add random if we have enough data
        )
        ...
        # Tell the optimizer all historical data
        xs_history = [item[0] for item in experiment_history]
        ys_history_internal = [item[1] for item in experiment_history]
        if xs_history:
            optimizer.tell(xs_history, ys_history_internal)

        # Reset convergence tracking for the new optimization phase
        patience_counter = 0
        if experiment_history:
            best_y_so_far_for_convergence = max(y for _, y in experiment_history) if maximize_objective else min(y for _, y in experiment_history)
        else:
        ...
        update_status("✅ Warm restart complete. Ready for new suggestions.")
        set_ui_state(phase='optimization')
        suggest_button.disabled = False # Re-enable suggest button
        warm_restart_button.disabled = False # Keep warm restart button enabled

    except Exception as e:
        update_status(f"❌ Error during warm restart: {e}", is_error=True)
        set_ui_state(phase='optimization') # Return to optimization state, but with error
...
def reset_all():
    ...
    global patience_counter, best_y_so_far_for_convergence # Reset convergence state
...
# Add Warm Restart button
warm_restart_button = Button(label="Perform Warm Restart", button_type="primary", width=400, disabled=True)
...
all_buttons = [lock_setup_button, suggest_button, submit_result_button, reset_button, warm_restart_button] # Include warm restart button
...
warm_restart_button.on_click(perform_warm_restart) # Attach callback for warm restart button
...
controls_col = column(
    ...
    warm_restart_button, # Add warm restart button to the layout
